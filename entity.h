...
public:
	...
	__forceinline int& m_szLastPlaceName() {
		return get< int >(g_entoffsets.m_szLastPlaceName);
	}

	__forceinline int& m_bShouldGlow() {
		return get< int >(g_entoffsets.m_bShouldGlow);
	}
...
public:
	...
	PAD(0x2);					// 0x010A 
	float	m_flDuckAdditional;	// 0x118 
	PAD(0x21C);					// 0x010C
	...
	PAD(0x4);					// 0x0340
	float	m_flLandAnimMultiplier; // 0x124 
}; // size: 0x344
...
public:
	...
	__forceinline int& m_iPing(int index) {
		return *(int*)((uintptr_t)this + g_entoffsets.m_iPing + index * sizeof(int));
	}
	...
	__forceinline int& m_nForceBone() {
		return get< int >(g_entoffsets.m_nForceBone);
	}

	const char* Player::GetLastPlace() {
