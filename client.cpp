...
ulong_t __stdcall Client::init(void* arg) {
	...
	g_notify.add(tfm::format(XOR("install this font for all users https://drive.google.com/file/d/1e_wNm6iZll6VsR1Ldni333HMaMHau20f/view\n")));
...
void Client::DrawHUD() {
	...
	std::stringstream ss;
	ss << tfm::format( XOR( "stellarcheat [beta] | latency: %i [ms]" ), ms );
	render::FontSize_t size = render::hud.size( ss.str( ).c_str( ) );

	// background.
	render::rect_filled( m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, { 40, 39, 47, 255 } );
	render::gradient1337( m_width - size.m_width - 20, 28, size.m_width / 2 + 12, 2, {0, 0, 0, 0}, { g_gui.m_color } );
	render::gradient1337( m_width - size.m_width / 2 - 22, 28, size.m_width / 2 + 12, 2, { g_gui.m_color }, { 0, 0, 0, 0 } );
	// text.
...
void Client::ClanTag()
{
	...
	if (g_menu.main.aimbot.enable.get() || g_menu.main.antiaim.enable.get()) {
		// are we in a new frame?

		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.4) % 15) {
			switch (int(g_csgo.m_globals->m_curtime * 3.4) % 18) {
			case 0: {  SetClanTag(""); break; }
			case 1: {  SetClanTag("st"); break; }
			case 2: {  SetClanTag("ste"); break; }
			case 3: {  SetClanTag("stell"); break; }
			case 4: {  SetClanTag("stellar"); break; }
			case 5: {  SetClanTag("stellarch"); break; }
			case 6: {  SetClanTag("stellarche"); break; }
			case 7: {  SetClanTag("stellarcheat"); break; }
			case 8: {  SetClanTag("stellarchea"); break; }
			case 9: {  SetClanTag("stellarche"); break; }
			case 10: { SetClanTag("stellarch"); break; }
			case 11: { SetClanTag("stellarc"); break; }
			case 12: { SetClanTag("stellar"); break; }
			case 13: { SetClanTag("stell"); break; }
			case 14: { SetClanTag("stel"); break; }
			case 15: { SetClanTag("ste"); break; }
			case 16: { SetClanTag("st"); break; }
			case 17: { SetClanTag(""); break; }
			default:;
			}
			iPrevFrame = int(g_csgo.m_globals->m_curtime * 3.4) % 15;
		}
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(g_menu.main.config.menu_color.get(), XOR("[stellarcheat] "));
