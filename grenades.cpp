...
void Grenades::paint() {
	...
	for (const auto& cur : m_path) {
		...
		if (render::WorldToScreen(prev, screen0) && render::WorldToScreen(cur, screen1))
			render::line(screen0, screen1, g_menu.main.visuals.grenade_tracer_color.get());
			render::line(screen0 - vec2_t(1, 1), screen1 - vec2_t(1, 1), g_menu.main.visuals.grenade_tracer_color.get());
	...
	if (target.second) {
		...
		if (render::WorldToScreen(m_path.front(), screen))
			render::esp_grenade.string(screen.x, screen.y - 15, { 255, 255, 255, 0xb4 }, tfm::format(XOR("-%i"), (int)target.first), render::ALIGN_CENTER);
	}
	...
	if (g_menu.main.visuals.tracers_circle.get()) {
		if (g_cl.m_weapon->m_iItemDefinitionIndex() == SMOKE)
