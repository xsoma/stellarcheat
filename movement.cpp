...
void Movement::Fast_stop() {
	...
	if (g_cl.m_local->m_vecVelocity().length_2d() > 1.0f && g_cl.m_local->m_vecVelocity().length_2d() < 250.0f && g_cl.m_flags & FL_ONGROUND)
	{
		g_movement.Stop(0.0);
	}
}
...
void Movement::AutoStopFIX()
{
	...
	if (can_stop && g_aimbot.m_stop) {
		if (g_menu.main.aimbot.minimal_damage.get() > 1) {
			if (g_menu.main.aimbot.minimal_damage.get() - 5 > 1 >= g_aimbot.m_damage) {
				Movement::QuickStop();
			}
		}
		else if (g_menu.main.aimbot.penetrate_minimal_damage.get() > 1) {
			if (g_menu.main.aimbot.penetrate_minimal_damage.get() - 2 > 1 >= g_aimbot.m_damage) {
				Movement::QuickStop();
			}
		}
		else if (g_menu.main.aimbot.minimal_damage_hp.get()) {
			Movement::QuickStop();
		}
		else if (g_menu.main.aimbot.penetrate_minimal_damage_hp.get()) {
			Movement::QuickStop();
		}
	}
