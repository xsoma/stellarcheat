...
void HVH::AutoDirection() {
	...
	if (!target.player) {
		...
		m_auto = math::NormalizedAngle(m_view - 179.f);
		m_auto_dist = -1.f;
		return;
	}
	...
	angles.emplace_back(m_view - 179.f);
	angles.emplace_back(m_view + 90.f /*+ g_csgo.RandomFloat(-20.f, 20.f)*/);
	angles.emplace_back(m_view - 90.f /*+ g_csgo.RandomFloat(-20.f, 20.f)*/);
		...
		if (g_menu.main.antiaim.body_fake_stand_fakewalk.get()) {
			...
			if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air) && !g_input.GetKeyState(g_menu.main.movement.fakewalk.get())) {
				...
				if (stand) {
					...
					case 6:
						//g_cl.m_cmd->m_view_angles.y += 180.f;
						negative ? g_cl.m_cmd->m_view_angles.y += 85.f : g_cl.m_cmd->m_view_angles.y -= 93.f;
						negative = !negative;
						break;
					case 7:
						//g_cl.m_cmd->m_view_angles.y += 180.f;
						g_cl.m_cmd->m_view_angles.y += m_jitter_update2 ? -g_menu.main.antiaim.jitter_minus_angle_lby.get() : g_menu.main.antiaim.jitter_plus_angle_lby.get();
						break;
					}
				}
			...
			// there will be an lbyt update on this tick.
			if (stand) {
				g_cl.m_cmd->m_view_angles.z = 30.f;
				switch (g_menu.main.antiaim.body_fake_stand.get()) {
					// left.
				case 1:
					g_cl.m_cmd->m_view_angles.y += 110.f;
				...
				g_cl.m_cmd->m_view_angles.z = 0.f;
			}
		...
		// run normal aa code.
			...
			case 3: {
				...
				break;
				  ...
				  // random.
			case 4:
				g_cl.m_cmd->m_view_angles.y = Spin(0, negative);
				break;
...
void HVH::DoFakeAntiAim() {
	...
	*g_cl.m_packet = true;
	...
	case 8:
		...
		// fast spin
	case 9:
		g_cl.m_cmd->m_view_angles.y = m_direction + 90.f + std::fmod(g_csgo.m_globals->m_curtime * 720.f, 360.f);
		break;

		// twist spin
	case 10:
		g_cl.m_cmd->m_view_angles.y = m_direction + 90.f + std::fmod(g_csgo.m_globals->m_curtime * 360.f, 180.f);
		if (g_cl.m_cmd->m_view_angles.y == 1.f)
			g_cl.m_cmd->m_view_angles.y = m_direction + 180.f + std::fmod(g_csgo.m_globals->m_curtime * 360.f, 0.f);
		break;

		// random spin
	case 11:
		g_cl.m_cmd->m_view_angles.y = Spin(m_direction + 90.f, m_direction - 90.f);
		break;

		// bugged direction
	case 12:
		g_cl.m_cmd->m_view_angles.y += m_jitter_update ? -g_menu.main.antiaim.jitter_minus_angle.get() : g_menu.main.antiaim.jitter_plus_angle.get();
		break;
...
void HVH::UpdateJitter()
{
	...
	const auto jitterSpeed = g_menu.main.antiaim.jitter_speed.get() + g_csgo.m_cl->m_choked_commands;
...
void HVH::UpdateJitter2()
{
	if (!g_cl.m_processing)
		return;

	const auto jitterSpeed = g_menu.main.antiaim.jitter_speed2.get() + g_csgo.m_cl->m_choked_commands;
	...
	if (lastTick + jitterSpeed < g_cl.m_local->m_nTickBase() || lastTick > g_cl.m_local->m_nTickBase())
	{
		...
		m_jitter_update2 = !m_jitter_update2;
	}
}
	...
	if ((g_tickbase.m_shift_data.m_should_attempt_shift && ((!g_tickbase.m_shift_data.m_should_be_ready && g_tickbase.m_shift_data.m_prepare_recharge) || g_tickbase.m_shift_data.m_needs_recharge || g_tickbase.m_shift_data.m_should_be_ready) /*&& !m_fake_duck*/)) {
		*g_cl.m_packet = g_csgo.m_cl->m_choked_commands >= 1;
		...
		if ((g_menu.main.antiaim.lag_enable.get() && !g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN))) {
			...
			int variance = std::clamp((int)g_menu.main.antiaim.fluc_limit.get(), 1, 100);
		...
		if (g_menu.main.antiaim.revoler_disable.get() && g_cl.m_weapon_id == REVOLVER && (!(g_cl.m_buttons & IN_JUMP) || (g_cl.m_flags & FL_ONGROUND))) {
			*g_cl.m_packet = true;
		}
