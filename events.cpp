...
void events::bullet_impact(IGameEvent* evt) {
	if (g_cl.m_processing) {
		...
		float impact_appeared = g_csgo.m_globals->m_curtime;
		...
		if (attacker == g_csgo.m_engine->GetLocalPlayer()) {
			...
			//int complete = fabs(impact_appeared - g_csgo.m_globals->m_curtime) / 4;
			//int alpha = (1 - complete) * 255;

			//render::line(vec2_t(pos.x - 7, pos.y - 7), vec2_t(pos.x - 2, pos.y - 2), { 60, 180, 225,255 });
			//render::line(vec2_t(pos.x - 7, pos.y + 7), vec2_t(pos.x - 2, pos.y + 2), { 60, 180, 225,255 });
			//render::line(vec2_t(pos.x + 7, pos.y + 7), vec2_t(pos.x + 2, pos.y + 2), { 60, 180, 225,255 });
			//render::line(vec2_t(pos.x + 7, pos.y - 7), vec2_t(pos.x + 2, pos.y - 2), { 60, 180, 225,255 });

			if (g_menu.main.visuals.bullet_impacts.get()) {
				g_csgo.m_debug_overlay->AddBoxOverlay(pos, vec3_t(-1.5, -1.5, -1.5), vec3_t(1.5, 1.5, 1.5), ang_t(0, 0, 0), 0, 0, 255, 127, 4);
			}
		}
...
void events::player_death(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "player_death"))
	{
		...
		// reset opacity scale.
		g_visuals.m_opacities[index] = 0.f;
		g_visuals.m_draw[index] = false;
		...
		if (index == g_csgo.m_engine->GetLocalPlayer())
		{
			// reset state
			iLife = false;
			//g_notify.add(tfm::format(XOR("Local player is dead\n")));

			// update skins, crap gloves and hands on the ground, but only if skins enabled
			/*if (g_menu.main.config.enable_skins.get())
