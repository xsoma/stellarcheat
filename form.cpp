...
void Form::draw( ) {
	...
	// lbgt line
	render::gradient( m_x + 7, m_y + 7, 2, m_height - 7, color, { 0, 0, 0, 0 } ); // make lower size
	render::gradient( m_x + 7 + m_width - 14, m_y + 7, 2, m_height - 7, color, { 0, 0, 0, 0 } );
	...
	if ( !m_tabs.empty( ) ) {
		...
		for ( size_t i{}; i < m_tabs.size( ); ++i ) {
			int pos = tabs_area.x + ( i * ( tabs_area.w / m_tabs.size( ) ) );
			int pos_extra[ 8 ] = { 20, 20, 20, 23, 26, 26, 20, 20 };

			const auto& t = m_tabs[ i ];

			render::menu_shade.string( pos + pos_extra[ i ], tabs_area.y + 6,
		...
		if ( !m_active_tab->m_elements.empty( ) ) {
			...
			std::string text = tfm::format( XOR( "%s | %s | %s" ), "[stellarcheat]", "[build 8/13/2021]", "[beta]" );
