...
void Visuals::DrawDamageIndicator()
{
	...
	for (int i = 0; i < g_cl.DamageIndicator.size(); ++i)
	{
		...
		if (render::WorldToScreen(g_cl.DamageIndicator[i].Position, ScreenPosition))
		{
			...
			render::esp.string(ScreenPosition.x, ScreenPosition.y, { 255,255,255, alpha }, std::to_string(g_cl.DamageIndicator[i].iDamage).c_str());
		}
	}
}
...
void Visuals::think() {
	...
	if (g_menu.main.visuals.removes.get(4)
		...
		case 0:
			...
			break;
		case 1:
			render::gradient( w / 2, h / 2 - 114, 1, 100, Color( 255, 0, 0, 0 ), g_menu.main.visuals.scope_color.get( ) );
			render::gradient1337( w / 2 - 118, h / 2, 100, 1, Color( 255, 0, 0, 0 ), g_menu.main.visuals.scope_color.get( ) );
			render::gradient1337( w / 2 + 19, h / 2, 100, 1, g_menu.main.visuals.scope_color.get( ), Color( 255, 0, 0, 0 ) );
			render::gradient( w / 2, h / 2 + 15, 1, 100, g_menu.main.visuals.scope_color.get( ), Color( 255, 0, 0, 0 ) );
			break;
		}
	}
...
void Visuals::Spectators() {
	...
	int h = render::spectators.m_size.m_height;
	...
	for (size_t i{ }; i < spectators.size(); ++i) {
		...
		if (g_menu.main.misc.watermark.get() && g_cl.m_local->alive())
			render::spectators.string(g_cl.m_width - 15, (i * 20) + 14, { 255, 255, 255, 200 }, name, render::ALIGN_RIGHT);
		else
			render::spectators.string(g_cl.m_width - 15, (i * 20) - 14, { 255, 255, 255, 200 }, name, render::ALIGN_RIGHT);
	}
}
...
void Visuals::PenetrationCrosshair() {
	...
	if (g_cl.m_pen_data.m_damage > 1) {
		...
		render::rect(x - 1, y - 1, 1, 1, Color(0, 0, 0, 150));
		render::rect(x - 1, y + 1, 1, 1, Color(0, 0, 0, 150));
		render::rect(x + 1, y - 1, 1, 1, Color(0, 0, 0, 150));
		render::rect(x + 1, y + 1, 1, 1, Color(0, 0, 0, 150));
	}
...
void Visuals::draw(Entity* ent) {
	...
	else if ((g_menu.main.visuals.items.get() || g_menu.main.visuals.ammo.get() || g_menu.main.visuals.icon_item.get() || g_menu.main.visuals.distance.get()) && ent->IsBaseCombatWeapon() && !ent->dormant())
...
void Visuals::DrawProjectile(Weapon* ent) {
	...
	// find classes.
		...
		if (g_menu.main.visuals.proj_range_world_circle.get()) {
		...
		render::esp_small.string(screen.x, screen.y - 3, { 255,255,255,200 }, XOR("FIRE"), render::ALIGN_CENTER);
...
void Visuals::DrawItem(Weapon* item) {
	...
	if (g_menu.main.visuals.icon_item.get()) {
	...
	if (g_menu.main.visuals.items.get() && g_menu.main.visuals.distance.get())
		render::esp_small.string(screen.x, screen.y - 11, col, distance, render::ALIGN_CENTER);
	else if (!g_menu.main.visuals.items.get() && g_menu.main.visuals.distance.get())
...
void Visuals::OffScreen(Player* player, int alpha) {
	...
	if (!is_on_screen
		...
		g_csgo.m_surface->DrawSetColor(color_outline);
	...
	if (!g_cl.m_processing) {
		...
		for (size_t i{ impact_count }; i-- > 0; ) {
			...
			if (is_final_impact) {
				...
				switch (g_menu.main.visuals.beam_type.get()) {
				case 0:
				...
				beam_info.m_flWidth = g_menu.main.visuals.impact_beams_time.get() / 2; // eto scale, ne ydalai
				beam_info.m_flEndWidth = g_menu.main.visuals.impact_beams_time.get() / 2; // eto scale, ne ydalai
