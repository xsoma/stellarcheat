...
public:
	...
	__forceinline void add(const std::string& text, Color color = colors::white, float time = 8.f, bool console = true) {
		...
		m_notify_text.push_back(std::make_shared< NotifyText >(text, colors::white, time));
	...
	void think() {
		...
		for (size_t i{}; i < m_notify_text.size(); ++i) {
			...
			render::FontSize_t sizepush = render::hud.size(notify->m_text);

			render::rect_filled(x + 5, y + 6, 3, 11, colors::light_blue);	//+67 to header
			render::menu_shade.string(x + 10, y + 5, color, notify->m_text);
			y += size;
		}
	}
};
